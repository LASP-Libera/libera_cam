...
- Record what the write path does under both pairings, measured on scaled fixtures and written up in `doc/overview.md`. `DASK_SCHEDULER=distributed` works only with `XARRAY_NETCDF_ENGINE=netcdf4`; with the default `h5netcdf` it fails on an unpicklable file object, independently of geolocation (it reproduces with `use_geo: false`), and the engine switch is a workaround, not a diagnosis. Neither pairing releases blocks as it writes, so client memory grows with granule length: about 0.6 GB + 42 MB per frame on `netcdf4` + `distributed` against 2.8 GB + 106 MB per frame on the default path. That allows roughly 750 frames in 32 GB, about an hour of data, and leaves full-length granules to roadmap item 5.
