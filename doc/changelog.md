...
- Declare the per-pixel geometry this product needs as curryer `PixelField` members (`_PIXEL_FIELDS`), the way `libera_rad` declares its `GeometryField` set, and pass them to `pixel_geometry(fields=...)`. curryer returns a `{column: array}` dict keyed by those fields' columns, so `_PIXEL_VARIABLES` maps curryer column names straight to product variables with no rename table. The frame-coverage probe asks for `QUALITY_FLAGS` alone, which is all it reads, so it no longer computes the Sun ephemeris and the surface angles for every frame just to check kernel coverage.
- Pin `lasp-curryer` to the `spatial-pixel-geometry` branch (lasp/curryer#188, version 0.5.3) until it is released.
- Pin `chunksizes: [1, 512, 512]` on every three-dimensional variable. `h5py.guess_chunk` picks the chunk depth from the array shape, so it changed with granule length (2 frames at 60, 5 at 160, 16 at 1003) while Dask writes fixed-size blocks; at production sizes the two stopped dividing evenly and the write slowed. A depth of one image divides any block size. Measured on the 1003-frame July 11 granule: 85.9 to 48.6 minutes (1.77x) for 5.4% more file, and with four workers 5.14 to 2.02 s/frame end to end (2.54x). Requires the `chunksizes` tuple coercion from libera-utils 5.10.9, which the pin above carries.
...
- Record what the write path does under each pairing, measured on scaled fixtures and two real granules and written up in `doc/overview.md`. `DASK_SCHEDULER=distributed` now works with either engine: the `h5netcdf` + `distributed` failure was `libera_utils` handing `to_netcdf` an open file object, which the scheduler cannot pickle, and `libera_utils` passes a path from 5.10.9. `h5netcdf` is the better engine on every axis measured (176 s against 217 s and 40.5 against 47.3 MB/frame at 60 frames), and one `netcdf4` + `distributed` run in three failed with a `KeyError` race in which a worker reopened the output before the client's variable definitions were visible to it. Peak memory does **not** grow with granule length -- a 1003-frame granule peaks at 7.38 GB against 9.14 GB for 160 frames -- so the per-frame memory models and the "roughly 750 frames in 32 GB" cap in earlier drafts are withdrawn; they were measured with `ru_maxrss` on `RUSAGE_SELF`, which omits `distributed`'s workers. `DASK_MEMORY_LIMIT` must still be raised above its `8GB` default to write 60 frames under `distributed`. Write throughput does degrade with granule length (2.96 s/frame at 160 frames, 5.14 s/frame at 1003), most likely from HDF5 chunk misalignment as `h5py.guess_chunk` changes the chunk depth with granule shape.
...
- Correct the FSW image-header `azimuth_angle` units: the field is **degrees**, signed, not radians. On `DITL_3min`, whose azimuth stage is rotating at about 0.5 deg/s, the header angle and the AZROT-CK agree to 0.089-0.099 degrees and their frame-to-frame increments match to 0.4%; the previously reported -137.7 and 3.9 degree differences were artefacts of a spurious `np.degrees` conversion in the diagnostic. `doc/wfov_fsw_header_reference.md`, the diagnostic helper and the 0.2.7 note are corrected, and the integration test now asserts both the sub-degree agreement and the 1:1 increment ratio so a reintroduced conversion fails loudly. The residual +0.096 +/- 0.004 degree offset is equivalent to a 0.19 s timing difference at that slew rate; constant-angle and constant-time explanations are indistinguishable in this fixture (`LIBSDC-816`).
...
- Derive `Azimuth` from the AZROT-CK as the motor encoder angle (third 1-2-3 Euler angle of `LIBERA_BASE_COORD -> LIBERA_AZ_COORD`, degrees in `[0, 360)`) via `curryer.compute.spatial.frame_to_frame_euler`, matching `libera_rad`. This replaces the FSW image-header `azimuth_angle` as the product source: the CK is the authoritative pointing record and is what `libera_rad` uses. `jpss_only` writes 0 degrees; `use_geo: false` writes `-999`. Elevation is not reported: the camera is mounted on the azimuth stage and ELSCAN-CK is not a camera L1B input.
