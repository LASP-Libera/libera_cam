...
@pytest.mark.parametrize(
    ("integration_time", "variable_name"),
    [
        (IntegrationTime.LONG, "cnt_obs1"),
        (IntegrationTime.SHORT, "cnt_obs0")
    ]
)
def test_plot_observed_vs_true_plus_relative_difference(test_data_path, local_data_path,
                                                        integration_time, variable_name):
    ...
    obs_datetime = (datetime.datetime(1, 1, 1) +
                    datetime.timedelta(seconds=np.round(((obs_julian_day-1)*86400.0), decimals=0)))
