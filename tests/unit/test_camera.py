...
@pytest.mark.parametrize(
    ("use_exact", "expected_tolerance"),
    [
        (True, 1e-5),
        (False, 1.4)
    ]
)
def test_synthetic_convert_dn_to_radiance(test_data_path, local_data_path,
                                          use_exact, expected_tolerance):
    ...
    calculated_radiance = convert_dn_to_radiance(pixel_counts, IntegrationTime.SHORT,
                                                 use_synthetic=True, use_exact=use_exact)
