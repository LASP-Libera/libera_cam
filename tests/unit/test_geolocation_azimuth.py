...
def test_add_geolocation_logs_fsw_before_overwrite(mock_prefetch, mock_calc_chunk, mock_log_diff):
    geo = np.zeros((2, 2, 2, 3), dtype=np.float64)
    az = np.array([90.0, 180.0], dtype=np.float64)
    ...
    ds = xr.Dataset(
        data_vars={
            ...
            "azimuth_angle": (("camera_time",), fsw_rad.astype(np.float32)),
        },
...
@patch("libera_cam.geolocation._calculate_chunk_geolocation_packed")
@patch("libera_cam.geolocation.prefetch_kernels")
def test_add_geolocation_assigns_spice_azimuth(mock_prefetch, mock_calc_chunk):
    geo = np.zeros((2, 2, 2, 3), dtype=np.float64)
    az = np.array([10.0, 20.0], dtype=np.float64)
    az_plane = np.broadcast_to(az[:, None, None, None], (2, 2, 2, 1))
    mock_calc_chunk.return_value = np.concatenate([geo, az_plane], axis=-1)
