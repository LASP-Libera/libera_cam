...
def test_generate_dark_offset(integration_time, use_synthetic,
                              expected_mean, expected_std_dev):
    if use_synthetic:
        scale_factor = generate_dark_offset(integration_time,
                                            use_synthetic=True)
        assert scale_factor.shape == (PIXEL_COUNT_X,
                                      PIXEL_COUNT_Y)
        assert np.abs(scale_factor.mean()-expected_mean) < 1
        assert np.abs(scale_factor.std()-expected_std_dev) < 1
    else:
        with pytest.raises(NotImplementedError):
            generate_dark_offset(integration_time,
                                 use_synthetic=False)
